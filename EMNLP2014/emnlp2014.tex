...
On top of these phrase-based approaches, the use of synchronous grammar-based formalisms \cite{Lewis1968} allows syntactic information to be explicitly incorporated in the translation process, either by taking into account both structural and categorical information \cite{Galley2004,Zollmann2006}, e.g., the rule \emph{EXAMPLE} or only structural information in the form of single-category grammars \cite{Chiang2005}, e.g., the rule \emph{EXAMPLE}.  

Single-category grammars work well in some circumstances.
Their use alleviates data sparseness issues, allows the option of building partial translations that can then be combined serially, and eliminates the need for a noisy domain-specific parser.   
However, by using only one category significant amounts of syntactic and context-specific information is discarded. 
...
$\bullet$ Preterminal rules ($\rules_1$): $$a(h_1) \rightarrow \langle \alpha_1 b(h_2) \alpha_2, \beta_1 b(h_2) \beta_2$$ where $a,b \in \mathcal{N}$, $h_1,h_2 \in [m]$, $\alpha_1,\alpha_2 \in [n_s]^{\ast}$ and $\beta,\beta_2 \in [n_t]^{\ast}$. 

$\bullet$ Terminal rules ($\rules_0$): $a(h_1) \rightarrow \langle \alpha, \beta \rangle$ where $a \in \mathcal{N}$, $\alpha \in [n_t]^{\ast}$ and $\beta \in [n_s]^{\ast}$.
...
The corresponding theoretical guarantees from \newcite{cohen-14b} can also be generalized to the synchronous case trivially.  
...
\begin{small}
  \begin{center}
    \begin{tabular}{|l|p{0.45\columnwidth}rr|}
	  ...
	  & $m=16$ RI & 46.06 & 55.83 \\
	  & $m=16$ RI+Lex+Sm & X & Y \\ 
	  & $m=16$ RI+Lex+Len & 45.7 & 55.29 \\ 
	  & $m=24$ RI+Lex & 43 & 51.28 \\ 
	  & $m=32$ RI+Lex & 43.06 & 52.16 \\ \hline
	  \multirow{2}{*}{EM} & $m=8$ X Iter & 38.61 & 46.62 \\
	  & $m=16$ X Iter & 42.45 & 53.16 \\
	  & $m=24$ X Iter & 41.33 & 51.55 \\
	  & $m=32$ X Iter & X & Y \\
	  \hline
	\end{tabular}
  \end{center}
