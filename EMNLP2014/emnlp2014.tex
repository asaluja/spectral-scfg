...
The contribution of this paper is an algorithm for learning an implicit refinement of nonterminal categories in synchronous context-free grammar (SCFG), using a latent variable model.  
...
To estimate the parameters $t$ of an L-SCFG, a dataset composed of synchronous s-trees is required.\cjd{not required, but we assume we have it for simplicity's sake. we need to make clear what our assumptions are up front because this is quite different from how parameters are usually learned in translation models}
